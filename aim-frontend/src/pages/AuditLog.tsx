  ...
  const handleExportCSV = async () => {
    try {
      pushToast({
        ...
        description: 'Fetching matching audit records.',
        variant: 'default',
      });

      const res = await auditLogApi.list({ ...filters, limit: 9999, page: 1 });
      const exportEntries: AuditLogEntry[] = Array.isArray(res.data)
        ? res.data
        : (res.data as any)?.entries || [];

      const headers = ['Timestamp', 'Event Type', 'Agent ID', 'Agent Name', 'Actor Role', 'Details'];
      ...
      pushToast({
        ...
        description: `Exported ${exportEntries.length} audit trail records.`,
        variant: 'default',
      });
    } catch (err: any) {
      pushToast({
        title: 'Export Failed',
        description: 'Failed to generate audit log CSV export.',
        variant: 'destructive',
      });
    }
  };
