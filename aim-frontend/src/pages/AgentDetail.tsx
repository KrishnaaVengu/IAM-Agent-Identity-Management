...
export const AgentDetail: React.FC = () => {
 ...
 const requestScopeMut = useMutation({
   mutationFn: (scope: string) => agentsApi.requestScope(agentId || '', scope),
   onSuccess: () => {
     pushToast({ title: 'Request Submitted', description: 'Scope escalation request is pending approval.', variant: 'default' });
   },
   onError: () => {
     pushToast({ title: 'Request Failed', description: 'Could not submit scope request.', variant: 'destructive' });
   }
 });
 ...
 {scopeDef?.description || 'Scoped permission grant'}
   ...
   <div className="pt-4 border-t border-slate-100">
     <h4 className="text-xs font-semibold text-slate-800 mb-2">Request Additional Scope</h4>
     <form onSubmit={(e) => { e.preventDefault(); const val = (e.target as any).elements.scope.value; if(val) requestScopeMut.mutate(val); }} className="flex gap-2">
       <select name="scope" className="flex-1 px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs text-slate-800">
         <option value="">Select Scope...</option>
         {SCOPE_CATALOG.filter(s => !agent.approvedScopes.includes(s.id)).map(s => (
           <option key={s.id} value={s.id}>{s.id}</option>
         ))}
       </select>
       <button type="submit" disabled={requestScopeMut.isPending} className="px-3 py-2 bg-blue-600 hover:bg-blue-500 text-white text-xs font-semibold rounded-lg disabled:opacity-50 cursor-pointer">Request</button>
     </form>
   </div>
