...
export const ResponsePanel: React.FC<ResponsePanelProps> = ({ result, isLoading }) => {
 if (isLoading) {
 return (
 <div className="bg-white p-12 rounded-xl border border-slate-200 shadow-sm flex flex-col items-center justify-center text-center min-h-[360px] space-y-3 transition-colors">
 ...
 const isLockdown = result.reasonCode === 'SECURITY_LOCKDOWN' || result.statusCode === 423;

 return (
 <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden space-y-0 transition-colors">
 {isLockdown && (
   <div className="bg-red-600 text-white font-bold text-sm p-4 text-center border-b border-red-700 animate-pulse">
     🚨 SECURITY LOCKDOWN: Multiple unauthorized access attempts detected. Agent automatically suspended and token revoked.
   </div>
 )}
 {/* Header Bar */}
 <div
 ...
 <p className={`p-3 rounded-lg font-sans border ${isLockdown ? 'bg-red-100 text-red-900 border-red-200 font-bold' : 'bg-slate-100 text-slate-800 border-slate-200'} `}>
 {result.message}
 </p>
 </div>
